...
@section('title', 'Edit Supply Category - ' . $category->name)
...
<div class="space-y-6 max-w-3xl mx-auto w-full">
        ...
        <div class="flex items-center gap-3">
            ...
            <div>
                <div class="flex items-center gap-2">
                    <h1 class="text-xl font-bold text-slate-900 tracking-tight">Edit Supply Category</h1>
                    <span class="px-2.5 py-0.5 bg-indigo-50 text-indigo-700 font-mono font-bold text-xs rounded-lg border border-indigo-100">
                        {{ $category->code }}
                    </span>
                </div>
                <p class="text-xs text-slate-500">Update hardware classification, icon, and display preferences</p>
            </div>
        </div>
        <div class="flex items-center gap-2">
            <x-forms.button href="{{ route('admin.categories.show', $category) }}" variant="ghost" icon="fas fa-eye">
                View Details
            </x-forms.button>
    ...
    <div class="bg-white p-6 sm:p-8 rounded-2xl border border-slate-200/80 shadow-xs">
        <x-forms.form action="{{ route('admin.categories.update', $category) }}" method="PUT" class="space-y-6">
                    ...
                    <div>
                        ...
                        <select name="icon" required class="w-full text-xs rounded-xl border-slate-200 focus:border-indigo-500 focus:ring-indigo-500 py-2.5">
                            @foreach($icons as $class => $label)
                                <option value="{{ $class }}" {{ old('icon', $category->icon) == $class ? 'selected' : '' }}>{{ $label }} ({{ $class }})</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                    ...
                    <div>
                        ...
                        <select name="status" required class="w-full text-xs rounded-xl border-slate-200 focus:border-indigo-500 focus:ring-indigo-500 py-2.5">
                            @foreach($statuses as $val => $label)
                                <option value="{{ $val }}" {{ old('status', $category->status) == $val ? 'selected' : '' }}>{{ $label }}</option>
                            @endforeach
                        </select>
                    </div>
