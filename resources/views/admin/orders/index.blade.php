...
@section('content')
    <div class="space-y-6">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            ...
            <div>
                @if (auth()->user()
                        ?->canDo(['orders.create', 'manage_orders']))
                    <a href="{{ route('admin.orders.create') }}"
                        class="inline-flex items-center gap-2 px-4 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-semibold rounded-xl shadow-xs hover:shadow transition-all">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div class="bg-white p-5 rounded-2xl border border-slate-200/80 shadow-xs flex items-center gap-4">
                ...
                <div>
                    ...
                    <h3 class="text-2xl font-black text-slate-900 mt-0.5">
                        {{ number_format($pendingOrdersCount + $processingOrdersCount) }}</h3>
                </div>
            </div>
            <div class="bg-white p-5 rounded-2xl border border-slate-200/80 shadow-xs flex items-center gap-4">
                <div
            ...
            <div class="bg-white p-5 rounded-2xl border border-slate-200/80 shadow-xs flex items-center gap-4">
                <div
                    class="w-12 h-12 rounded-xl bg-indigo-50 text-indigo-600 flex items-center justify-center text-xl font-bold">
        ...
        <div class="border-b border-slate-200 flex gap-4 text-xs font-bold">
            <a href="{{ route('admin.orders.index') }}"
                class="pb-3 border-b-2 transition-colors {{ !isset($isCustomerRegisterOrders) ? 'border-indigo-600 text-indigo-600' : 'border-transparent text-slate-500 hover:text-slate-700' }}">
                <i class="fas fa-handshake mr-1.5"></i> Customer Partner Orders
            </a>
            <a href="{{ route('admin.orders.registered') }}"
                class="pb-3 border-b-2 transition-colors {{ isset($isCustomerRegisterOrders) ? 'border-indigo-600 text-indigo-600' : 'border-transparent text-slate-500 hover:text-slate-700' }}">
        ...
        <div class="bg-white p-4 rounded-2xl border border-slate-200/80 shadow-xs">
            <form method="GET"
                action="{{ isset($isCustomerRegisterOrders) ? route('admin.orders.registered') : route('admin.orders.index') }}"
                ...
                <div>
                    <select name="status" onchange="this.form.submit()"
                        class="w-full py-2 px-3 bg-slate-50 border border-slate-200 rounded-xl text-xs text-slate-800 focus:bg-white focus:border-indigo-500 focus:outline-none">
                            ...
                            <tr class="hover:bg-slate-50/80 transition-colors">
                                ...
                                <td class="px-5 py-4">
                                    <a href="{{ route('admin.orders.show', $order->id) }}"
                                        class="font-bold text-indigo-600 hover:underline text-sm">
                                        {{ $order->order_number }}
                                    </a>
                                    @if ($order->order_source === 'frontend')
                                        <span
                                            class="inline-block px-1.5 py-0.5 rounded text-[10px] font-bold bg-purple-50 text-purple-700 border border-purple-200 ml-1">
                                            <i class="fas fa-globe text-[9px] mr-1"></i>Frontend
                                        </span>
                                    @else
                                        <span
                                            class="inline-block px-1.5 py-0.5 rounded text-[10px] font-bold bg-indigo-50 text-indigo-700 border border-indigo-200 ml-1">
                                            <i class="fas fa-handshake text-[9px] mr-1"></i>Partner Order
                                        </span>
                                    @endif
                                    <p class="text-[11px] text-slate-400 mt-0.5">
                                        <i
                                            class="far fa-calendar-alt mr-1"></i>{{ $order->order_date ? $order->order_date->format('M d, Y') : 'N/A' }}
                                    </p>
                                </td>
                                ...
                                <td class="px-5 py-4">
                                    <div class="flex items-center gap-3">
                                        <div
                                            class="w-8 h-8 rounded-full bg-slate-100 text-slate-600 flex items-center justify-center font-bold text-xs shadow-xs border border-slate-200">
                                        ...
                                        <div>
                                            ...
                                            <div class="text-[11px] text-slate-500 flex items-center gap-2">
                                                ...
                                                @if (!empty($order->customer->tier))
                                                    <span
                                                        class="px-1.5 py-0.2 rounded text-[10px] font-semibold bg-amber-50 text-amber-700 border border-amber-200">
                                                        {{ $order->customer->tier }}
                                                    </span>
                                                @endif
                                            </div>
                                ...
                                <td class="px-5 py-4">
                                    ...
                                    <div class="text-[11px] text-slate-400">
                                        ...
                                        @if ($order->discount_amount > 0)
                                            <span
                                                class="text-emerald-600 font-semibold">(-${{ number_format($order->discount_amount, 2) }})</span>
                                        @endif
                                    </div>
                                </td>
                                ...
                                <td class="px-5 py-4">
                                    <span
                                        class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[11px] font-bold border {{ $order->payment_badge }}">
                                        <span class="w-1.5 h-1.5 rounded-full bg-current"></span>
                                        {{ ucfirst(str_replace('_', ' ', $order->payment_status)) }}
                                    </span>
                                    <p class="text-[10px] text-slate-400 mt-1">Terms:
                                        {{ $order->payment_terms ?? 'Standard' }}</p>
                                </td>

                                <!-- Order Fulfillment Status -->
                                <td class="px-5 py-4">
                                    <span
                                        class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[11px] font-bold border {{ $order->status_badge }}">
                                        <span class="w-1.5 h-1.5 rounded-full bg-current"></span>
                                ...
                                <td class="px-5 py-4 text-right">
                                    <div class="inline-flex items-center gap-1">
                                        <a href="{{ route('admin.orders.show', $order->id) }}"
                                            class="inline-flex items-center gap-1 px-2.5 py-1.5 bg-slate-100 hover:bg-indigo-50 hover:text-indigo-600 text-slate-700 font-semibold rounded-lg transition-colors text-xs"
                                            title="View Details">
                                        ...
                                        @if (auth()->user()
                                            ...
                                            <a href="{{ route('admin.orders.edit', $order->id) }}"
                                                class="inline-flex items-center gap-1 px-2.5 py-1.5 bg-amber-50 hover:bg-amber-100 text-amber-700 font-semibold rounded-lg transition-colors text-xs"
                                                title="Edit Order">
                                        ...
                                        @if (auth()->user()
                                            ...
                                            <form action="{{ route('admin.orders.destroy', $order->id) }}" method="POST"
                                                ...
                                                @method('DELETE')
                                                <button type="submit"
                                                    class="inline-flex items-center gap-1 px-2 py-1.5 bg-rose-50 hover:bg-rose-100 text-rose-700 font-semibold rounded-lg transition-colors text-xs"
                                                    title="Delete Order">
                        ...
                        @empty
                            <tr>
                                <td colspan="6" class="px-5 py-12 text-center text-slate-400">
                                    ...
                                    <p class="font-bold text-slate-600 text-sm">No
                                        {{ isset($isCustomerRegisterOrders) ? 'Customer Register Frontend' : 'Wholesale Customer Partner' }}
                                        Orders Found</p>
