    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            <div class="flex items-center gap-3">
                ...
                <span class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold border {{ $order->status_badge }}">
        ...
        <div class="flex items-center gap-3">
            ...
            <form action="{{ route('admin.orders.destroy', $order->id) }}" method="POST" class="inline" onsubmit="return confirm('Are you sure you want to delete order #{{ $order->order_number }}? Stock quantities will be restored.');">
                @csrf
                @method('DELETE')
                <button type="submit" class="px-4 py-2 bg-rose-50 hover:bg-rose-100 text-rose-700 text-xs font-semibold rounded-xl border border-rose-200 transition-colors">
                    <i class="fas fa-trash mr-1"></i> Delete
                </button>
            </form>
            <button onclick="window.print()" class="px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-semibold rounded-xl shadow-xs transition-all">
        ...
        <form action="{{ route('admin.orders.update-status', $order->id) }}" method="POST" class="flex flex-col sm:flex-row items-center justify-between gap-4">
            ...
            <div class="flex items-center gap-3">
                ...
                <div>
                    ...
                    <p class="text-xs text-slate-500">Change order processing stage or payment status for this wholesale order</p>
                </div>
            </div>
            <div class="flex items-center gap-3 w-full sm:w-auto">
                <select name="status" class="px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl text-xs font-semibold text-slate-800 focus:outline-none">
                ...
                <select name="payment_status" class="px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl text-xs font-semibold text-slate-800 focus:outline-none">
                    <option value="unpaid" {{ $order->payment_status === 'unpaid' ? 'selected' : '' }}>Unpaid</option>
                    <option value="partially_paid" {{ $order->payment_status === 'partially_paid' ? 'selected' : '' }}>Partially Paid</option>
                    <option value="paid" {{ $order->payment_status === 'paid' ? 'selected' : '' }}>Paid</option>
                </select>

                <button type="submit" class="px-4 py-2 bg-slate-900 hover:bg-black text-white text-xs font-bold rounded-xl shadow-xs transition-colors">
                    Update Status
                </button>
            </div>
        </form>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div class="bg-white p-6 rounded-2xl border border-slate-200/80 shadow-xs space-y-4">
            ...
            <div>
                ...
                <p class="text-xs font-semibold text-indigo-600">{{ $order->customer->company_name ?? 'Individual' }}</p>
                <p class="text-xs text-slate-500 mt-1">Code: <span class="font-mono font-bold">{{ $order->customer->customer_code ?? 'N/A' }}</span></p>
            </div>
            ...
            <div class="space-y-2 text-xs text-slate-600 border-t border-slate-100 pt-3">
                ...
                <div class="flex justify-between">
                    ...
                    <span class="font-bold text-emerald-600">{{ $order->customer->wholesale_discount ?? 0 }}% OFF</span>
                </div>
                <div class="flex justify-between">
        ...
        <div class="bg-white p-6 rounded-2xl border border-slate-200/80 shadow-xs space-y-4 md:col-span-2">
            ...
            <div class="grid grid-cols-2 gap-4 text-xs">
                <div>
                    ...
                    <span class="font-bold text-slate-800 bg-slate-100 px-2.5 py-1 rounded-lg inline-block">{{ $order->payment_terms ?? 'Net 30' }}</span>
                </div>
                <div>
                    <span class="text-slate-400 block mb-1">Shipping Address</span>
                    <span class="font-semibold text-slate-800 block">{{ $order->shipping_address ?? 'Standard Warehouse Delivery' }}</span>
                </div>
            </div>
                ...
                <tbody class="divide-y divide-slate-100 font-medium text-slate-700">
                    @foreach($order->items as $index => $item)
                        <tr>
                            ...
                            <td class="px-5 py-3 text-right font-bold text-slate-900">${{ number_format($item->subtotal, 2) }}</td>
                        </tr>
                    @endforeach
                </tbody>
            </table>
