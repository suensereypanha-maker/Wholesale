...
@section('title', 'Edit Product - ' . $stock->product_name)
...
<div class="w-full space-y-6">
    ...
    <div class="flex items-center justify-between gap-4 bg-white p-6 rounded-2xl border border-slate-200/80 shadow-xs">
        <div class="flex items-center gap-3">
            ...
            <div>
                <h1 class="text-xl font-bold text-slate-900 tracking-tight">Edit Product: {{ $stock->product_name }}</h1>
                <p class="text-xs text-slate-500">Update product descriptions, CPU specifications, pricing structure, and warehouse stock levels for SKU: {{ $stock->sku }}</p>
            </div>
        </div>
    ...
    <!-- Edit Form Card -->
    <x-forms.card title="Modify Product & Stock Configuration" subtitle="Update item parameters for SKU: {{ $stock->sku }}">
        <x-forms.form action="{{ route('admin.stocks.update', $stock) }}" method="PUT" hasFiles>
            ...
            <div class="space-y-6">
                ...
                <div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        >
                            @foreach($categories as $cat)
                                <option value="{{ $cat }}" {{ old('category', $stock->category) == $cat ? 'selected' : '' }}>{{ $cat }}</option>
                            @endforeach
                        </x-forms.select>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mt-4">
                        <x-forms.file 
                            ...
                            :value="$stock->image"
                            :preview="$stock->image_url"
                            helpText="Upload new product image to replace existing thumbnail" 
                        />
                        ...
                        <x-forms.input 
                            ...
                            :value="old('short_description', $stock->short_description)" 
                            helper="Brief summary shown in product list & cards"
                        />
                    </div>
                    ...
                    <div class="mt-4">
                        <x-forms.textarea 
                            ...
                            placeholder="Detailed product descriptions, technical details, features..." 
                            rows="4" 
                        >{{ old('description', $stock->description) }}</x-forms.textarea>
                    </div>
                </div>
                ...
                @php
                    $existingSpecs = [];
                    if (!empty($stock->details) && is_array($stock->details)) {
                        foreach ($stock->details as $specK => $specV) {
                            $existingSpecs[] = ['key' => str_replace('_', ' ', ucwords($specK)), 'value' => $specV];
                        }
                    }
                @endphp
                ...
                <div x-data="{ cost: {{ old('unit_cost', $stock->unit_cost) }}, retail: {{ old('retail_price', $stock->retail_price) }} }">
                    <h3 class="text-xs font-bold uppercase tracking-wider text-slate-400 mb-3 flex items-center gap-2">
                    ...
                    <div class="grid grid-cols-2 sm:grid-cols-3 xl:grid-cols-6 gap-4">
                        ...
                        <x-forms.input 
                            ...
                            :value="old('retail_price', $stock->retail_price)" 
                            x-model="retail"
                            required 
                        />
                        ...
                        <x-forms.input 
                            ...
                            label="Low Stock Min" 
                            :value="old('min_reorder_level', $stock->min_reorder_level)" 
                            required 
                        />
                        ...
                        <x-forms.input 
                            ...
                            name="max_capacity" 
                            label="Max Rack Cap" 
                            :value="old('max_capacity', $stock->max_capacity)" 
                            required 
                        />
                    </div>
                    ...
                    @php
                        $savedTiers = !empty($stock->tier_prices) && is_array($stock->tier_prices) ? $stock->tier_prices : [
                            ['min_qty' => 1, 'max_qty' => 5, 'price' => $stock->retail_price],
                            ['min_qty' => 6, 'max_qty' => 100, 'price' => round($stock->retail_price * 0.9, 2)],
                            ['min_qty' => 101, 'max_qty' => '', 'price' => round($stock->retail_price * 0.8, 2)],
                        ];
                    @endphp
                    <div class="mt-6 bg-slate-50/80 p-4 rounded-xl border border-slate-200/80" x-data="{
                        tiers: {{ json_encode($savedTiers) }},
                        addTier() {
                        ...
                        <div class="flex items-center justify-between mb-3">
                            <div>
                                ...
                                <p class="text-[11px] text-slate-500">Configure bulk order quantity price tiers for SKU: {{ $stock->sku }}</p>
                            </div>
                                ...
                                <div class="flex items-center gap-4 bg-white px-3.5 py-2 rounded-lg border border-slate-200 text-xs shadow-2xs">
                                    ...
                                    <div class="flex items-center gap-4 flex-1">
                                        ...
                                        <div class="flex items-center gap-2 flex-1">
                                            ...
                                            <input type="number" step="0.01" name="tier_price[]" x-model="tier.price" min="0" placeholder="e.g. 320.00" required class="w-full text-xs font-extrabold text-indigo-900 rounded-lg border border-slate-300 focus:border-indigo-600 focus:ring-2 focus:ring-indigo-500/20 py-1.5 px-2.5 bg-white shadow-2xs" />
                                        </div>
                                    </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <x-forms.input 
                        ...
                        :value="old('rack_location', $stock->rack_location)" 
                    />

                    <x-forms.textarea 
                        name="notes" 
                        label="Inventory Line Notes & Logs" 
                        rows="2" 
                    >{{ old('notes', $stock->notes) }}</x-forms.textarea>
                </div>
            </div>
