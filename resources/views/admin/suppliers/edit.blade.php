...
<div class="space-y-6 max-w-4xl mx-auto w-full">
        ...
        <div class="flex items-center gap-3">
            ...
            <div>
                <div class="flex items-center gap-2">
                    <h1 class="text-xl font-bold text-slate-900 tracking-tight">Edit Supplier</h1>
                    <span class="px-2.5 py-0.5 bg-indigo-50 text-indigo-700 font-mono font-bold text-xs rounded-lg border border-indigo-100">
                        {{ $supplier->code }}
                    </span>
                </div>
                <p class="text-xs text-slate-500">Update supplier record, payment terms, and contact representative</p>
            </div>
        </div>
        <div class="flex items-center gap-2">
            <x-forms.button href="{{ route('admin.suppliers.show', $supplier) }}" variant="ghost" icon="fas fa-eye">
                View Profile
            </x-forms.button>
    ...
    <div class="bg-white p-6 sm:p-8 rounded-2xl border border-slate-200/80 shadow-xs">
        <x-forms.form action="{{ route('admin.suppliers.update', $supplier) }}" method="PUT" class="space-y-6">
                    ...
                    <div>
                        ...
                        <select name="category" class="w-full text-xs rounded-xl border-slate-200 focus:border-indigo-500 focus:ring-indigo-500 py-2.5">
                            ...
                            @foreach($categories as $cat)
                                <option value="{{ $cat }}" {{ old('category', $supplier->category) == $cat ? 'selected' : '' }}>{{ $cat }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div>
                        ...
                        <select name="payment_terms" required class="w-full text-xs rounded-xl border-slate-200 focus:border-indigo-500 focus:ring-indigo-500 py-2.5">
                            @foreach($paymentTermsOptions as $term)
                                <option value="{{ $term }}" {{ old('payment_terms', $supplier->payment_terms) == $term ? 'selected' : '' }}>{{ $term }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div>
                        ...
                        <select name="rating" class="w-full text-xs rounded-xl border-slate-200 focus:border-indigo-500 focus:ring-indigo-500 py-2.5">
                            @for($r = 5; $r >= 1; $r--)
                                <option value="{{ $r }}" {{ old('rating', $supplier->rating) == $r ? 'selected' : '' }}>{{ $r }} Stars</option>
                            @endfor
                        </select>
                    </div>
                </div>
                    ...
                    <div>
                        ...
                        <select name="status" required class="w-full text-xs rounded-xl border-slate-200 focus:border-indigo-500 focus:ring-indigo-500 py-2.5">
                            @foreach($statuses as $value => $label)
                                <option value="{{ $value }}" {{ old('status', $supplier->status) == $value ? 'selected' : '' }}>{{ $label }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
